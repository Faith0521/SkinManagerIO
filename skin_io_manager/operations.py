...
def exportSkinPack(packPath, objs, versioning=False, file_ext=".gSkin"):
    ...
    for obj in objs:
        ...
        # if file_ext != ".npz" and skin.exportSkin(filePath, [obj]):
...
@timing
def exportSkin(folder_path, objs, versioning=False, file_ext=".npz", prevent_unsupported_method=True):
...
@timing
def importSkin(folderPath, objs=[], file_ext='.npz', createMissingJoints=False, skipAlreadySkinned=True):
